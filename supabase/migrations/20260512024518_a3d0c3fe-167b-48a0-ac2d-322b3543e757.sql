...
CREATE POLICY "Authenticated view active seat locks" ON public.seat_locks FOR SELECT TO authenticated USING (expires_at > now());
CREATE POLICY "Users create own seat locks" ON public.seat_locks FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users delete own seat locks" ON public.seat_locks FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY "Authenticated insert audit logs" ON public.audit_logs FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id OR user_id IS NULL);
...
ALTER TABLE public.flights ADD COLUMN IF NOT EXISTS is_active boolean NOT NULL DEFAULT true;
CREATE INDEX IF NOT EXISTS idx_flights_active_departure ON public.flights (is_active, departure_time);
