 ...
 * Flight admins, super admins (and legacy admins) can list, create, edit,
 * and enable/disable flights. Server functions enforce the same checks.
 */
...
function AdminFlightsPage() {
  ...
  const allowed = hasAnyRole(["super_admin", "admin", "flight_admin"]);
  ...
  return (
    <div className="container mx-auto px-4 py-16 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        You need the Flight Admin or Super Admin role to view this page.
      </p>
    </div>
  );
}
...
function AdminFlightsManager() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-10">
      ...
      <FlightDialog open={creating} onOpenChange={setCreating} mode="create" onSaved={loadFlights} />
      <FlightDialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)} mode="edit" flight={editing ?? undefined} onSaved={loadFlights} />
    </div>
  );
}
...
function FlightRowItem({ flight, onEdit, onChanged }: { flight: FlightRow; onEdit: () => void; onChanged: () => void }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {flight.is_active ? <Badge variant="secondary">Active</Badge> : <Badge variant="outline" className="text-muted-foreground">Disabled</Badge>}
      </TableCell>
...
function FlightDialog({ open, onOpenChange, mode, flight, onSaved }: { open: boolean; onOpenChange: (o: boolean) => void; mode: "create" | "edit"; flight?: FlightRow; onSaved: () => void; }) {
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && flight) {
      ...
      setFlightNumber(""); setOrigin(""); setDestination(""); setDeparture(""); setArrival(""); setBasePrice(""); setTotalSeats("60");
    }
  }, [open, mode, flight]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>{mode === "create" ? "Add a new scheduled flight." : "Update the flight schedule and fare."}</DialogDescription>
        </DialogHeader>
